...
class NativeProcessor(
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        logger.info("Process options=$options")
        // only generates code for single common module
        if (options["target"] != "common") return emptyList()
        logger.info("Processing @NativeData annotations")
        val symbols = resolver
            .getSymbolsWithAnnotation("NativeData")
    ...
    private fun generate(declaration: KSClassDeclaration) {
        ...
        var offset = "index + 0"
        declaration.getAllProperties().forEachIndexed { i, prop ->
        ...
        val stackFunBuilder = FunSpec.builder(generatedDataName)
            .receiver(ClassName(pkg, "NativeStack"))
            .returns(ClassName(pkg, generatedDataName))
        ...
        val constructorParams = fields.map { it.name }.joinToString(",")

        stackFunBuilder.addCode(
            """
            return $generatedDataName($constructorParams push($generatedDataName.SIZE_BYTES))
            """.trimIndent()
        )
...
//            val property = PropertySpec.builder(field.name, type)
